...
uint8_t LaserDriver::intensity = 10; // Intensity to use for next move queued if we want lasers. This is NOT the current value!
bool LaserDriver::laserOn = false;
void LaserDriver::initialize() {
    if(EVENT_INITALIZE_LASER) {
#if LASER_PIN > -1
        Com::printFLN(PSTR("Laser Initialize"));
        SET_OUTPUT(LASER_PIN);
        WRITE(LASER_PIN,!LASER_ON_HIGH);
        SET_OUTPUT(LASER_INTENSITY_PIN);
#endif
    }
    changeIntensity(0);
}
...
#if LASER_PIN > -1
        Com::printFLN(PSTR("Change Laser intensity = "), newIntensity);
        analogWrite(LASER_INTENSITY_PIN, newIntensity);
#endif
    }
}
#endif // SUPPORT_LASER
...
void CNCDriver::initialize() {
    ...
    {
#if CNC_ENABLE_PIN > -1
        Com::printFLN(PSTR("CNC Initialize"));
...
/** Turns laser on. Default implementation uses a enable pin LASER_PIN. rpm is ignored.
To override with event system, return false for the event
EVENT_LASER(rpm)
*/
void LaserDriver::laserOnCW(int32_t intensity) {
    Com::printFLN(PSTR("Laser On."));
    laserOff();
    CNCDriver::direction = 1;
    if(EVENT_LASER_ON(intensity)) {
#if LASER_PIN > -1
        WRITE(LASER_PIN, LASER_ON_HIGH);
...
/** Turns off laser. For event override implement
EVENT_LASER_OFF
returning false.
*/
void LaserDriver::laserOff() {
    if(CNCDriver::direction == 0) return; // already off
    Com::printFLN(PSTR("Laser Off."));
    if(EVENT_LASER_OFF) {
#if LASER_PIN > -1
        WRITE(LASER_PIN,!LASER_ON_HIGH);
        changeIntensity(0);
...
void CNCDriver::spindleOnCW(int32_t rpm) {
    if(direction == 1)
        return;
    Com::printFLN(PSTR("Spindle On CW = "), rpm);
    spindleOff();
    direction = 1;
...
void CNCDriver::spindleOnCCW(int32_t rpm) {
    if(direction == -1)
        return;
    Com::printFLN(PSTR("Spindle On CCW = "), rpm);
    spindleOff();
    direction = -1;
    if(EVENT_SPINDLE_CW(rpm)) {
#if CNC_DIRECTION_PIN > -1
...
/** Set spindle speed.
*/
void CNCDriver::spindleSpeed(int32_t rpm) {
#if CNC_ENABLE_PIN > -1
    Com::printFLN(PSTR("Spindle Speed = "), rpm);
    uint8_t nrServo = 0;
    int ms = (int)map(rpm, 0, 12000, 2130, 3770);
    if (ms > 3770) ms = 3770;
    if (rpm == 0) ms = 1870;
    uint16_t autoOff = 0;
    HAL::wrtMicroseconds(nrServo, ms, autoOff);
#endif
}
#endif
