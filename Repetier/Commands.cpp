        ...
        case 1: // G1
...
#if defined(SUPPORT_LASER) && SUPPORT_LASER
            {
                ...
                if(com->G == 0) {
                    ...
                    if (Printer::mode == PRINTER_MODE_LASER) {
                        LaserDriver::changeIntensity(map(com->S, 0, LASER_MAX_FILE, 0, LASER_MAX_INTENSITY));
                    }
                }
#endif // defined
...
//                LaserDriver::laserOn = laserOn;
...
#if defined(SUPPORT_CNC) && SUPPORT_CNC
            {
                if (Printer::mode == PRINTER_MODE_CNC) {
                    if(com->hasS()) {
                      if (CNCDriver::direction == 0) {
                        CNCDriver::spindleOnCW(0);
                      }
                      CNCDriver::spindleSpeed(com->S);
                    }
                }
            }
#endif // defined
    ...
    switch( com->M ) {
        case 3: // Spindle/laser on
            ...
            if(Printer::mode == PRINTER_MODE_LASER) {
                ...
                LaserDriver::laserOnCW(com->hasS() ? map(com->S, 0, LASER_MAX_FILE, 0, LASER_MAX_INTENSITY) : 0);
